...
```text
When a user request includes "뭔말알?", "뭔말인지 알지?", "이 느낌 알지?", "이 방향 맞지?", "감 잡았지?", "대충 이런 거", "알아서 잘", "찰떡같이", "내 의도 알겠지?", or English equivalents like "you know what I mean?", "get the vibe?", "get the direction?", "something like this", "use your judgment", or "make it fit", activate nah-mean intent alignment.

Do not execute immediately. First align:
1. explicit request
...
Ask at most 1 to 3 narrowing questions only when needed. Prefer reasonable defaults over question lists. Wait for confirmation before target-task execution.

If the user asks to skip confirmation with "바로 해", "확인 생략", "질문하지 말고 진행", "just do it", "skip confirmation", or "proceed without asking", give one short inline alignment and execute.
