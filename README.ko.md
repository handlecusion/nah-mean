...
유쾌한 이름이지만 성능 기준은 진지하게 잡는다. 벤치마크 기준으로 v0.3.1은 skill 없는 vanilla agent보다 rework-adjusted 예상 시간을 평균 49.8% 줄였다. 단, vanilla는 skill context와 alignment 응답이 없기 때문에 upfront token overhead는 더 낮다.
...
v0.3.1은 같은 6개 fixture를 skill 없는 vanilla agent와 비교한다. 이 벤치는 model quality benchmark가 아니라 deterministic protocol-risk estimate다.
...
| Expected rework units | 4.6 | 0.8 | 81.8% 감소 | nah-mean |
| Coordination workload units | 4.6 | 2.8 | 38.2% 감소 | nah-mean |
| Rework-adjusted 예상 시간(초) | 36.7 | 18.4 | 49.8% 감소 | nah-mean |

Input context와 upfront alignment token은 vanilla보다 나쁘다. 대신 expected rework, coordination workload, rework-adjusted time 중 하나 이상이 30% 이상 줄었는지 gate로 검증한다.
