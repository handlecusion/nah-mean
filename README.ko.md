...
```text
감다뒤 인식.
내가 잡았던 의도: 빠르게 쓸 수 있는 짧은 수정.
어긋난 지점: 사용자가 원한 건 속도보다 방향 재정렬인데, 결과만 고치려 했다.
다시 맞출 sight: 왜 마음에 안 들었는지보다 어떤 기준으로 다시 봐야 하는지 먼저 잡는다.
재작업 기준: 이전 결과를 방어하지 않고, 새 기준에 맞춰 범위와 톤을 다시 정한다.
알잘딱 route: Edit.
...
Vanilla agent는 skill context와 alignment 응답이 없어서 upfront token overhead가 더 낮다. nah-mean은 모호한 요청에서 짧은 intent contract가 잘못된 범위의 파일 수정, generic design, 얕은 리서치, 톤/스타일 재작업을 줄일 가능성이 높을 때 쓰는 쪽이 맞다.
